...
impl CliEngine {
    ...
    fn interactive_create(&self) -> Result<Commands> {
        ...
        let category = self.config.get_category(&category_name)
        ...
        if options.is_empty() {
            return Err(anyhow!("Category '{}' has no available items", category_name));
        }
        ...
        let (item_type, item_name) = if selected_item == "Create new (dynamic)" {
            ...
            if let Some(default_structure) = category.get_default_structure() {
                ...
                return Err(anyhow!("Category '{}' supports dynamic children but has no default structure", category_name));
            }
        } else {
    ...
    pub fn handle_create(&self, cmd: Commands) -> Result<()> {
        if let Commands::Create { category, item, name } = cmd {
            ...
            println!("🏗️  Creating {} '{}' in category '{}'...", item_type, item_name, category_name);
            ...
            let item_config = if let Some(static_item) = category.get_item(&item_type) {
                ...
                let default_structure = category.get_default_structure()
                    .ok_or_else(|| anyhow!("Category '{}' supports dynamic children but has no default structure", category_name))?;
                
                default_structure.get(&item_type)
                    .ok_or_else(|| anyhow!("Item type '{}' not found in default structure", item_type))?
            } else {
                return Err(anyhow!("Item type '{}' not found in category '{}'", item_type, category_name));
            };
            
            // Create the item using Creator
            self.create_item_with_config(&category_name, &item_type, &item_name, &item_config)?;
            
            println!("✅ Successfully created {} '{}' in {}/{}", item_type, item_name, category_name, item_type);
        ...
        for category_name in self.config.get_categories() {
            if let Some(category) = self.config.get_category(&category_name) {
                ...
                if category.supports_dynamic_children() {
                    if let Some(default_structure) = category.get_default_structure() {
                        let dynamic_types: Vec<String> = default_structure.keys().cloned().collect();
    ...
    fn create_item_with_config(
        ...
        let file_path = item_path.join("index").with_extension(&item_config.file_extension);
